...
#include "shared/text/string.h"            // for stringOccurrenceOfFrom, string
...
void pathCreate(String fileLocation, Arena scratch) {
    U32 currentIndex = 0;
    I64 slashIndex = stringOccurrenceOfChar(fileLocation, '/', .from = currentIndex);
    if (slashIndex >= 0) {
        ...
        while (slashIndex > 0) {
            ...
            slashIndex =
                stringOccurrenceOfChar(fileLocation, '/', .from = currentIndex);
        }
    }
}
